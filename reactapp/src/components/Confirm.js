...
export class Confirm extends Component {
    ...
    render() {
        const { values:{ name, email, phone, password, facebook, twitter, github } }=this.props;

        return (
            <div className="form-container">
                <h1 className="mb-5">Confirm</h1>
                <ul class="list-group">
                <li class="list-group-item disabled">Name: {name}</li>
                ...
                <button className="btn btn-danger" onClick={this.back}>Back</button>
                    ...
                    <button className="btn btn-primary" onClick={this.continue}>Contunie</button>
                    </div>
                    </div>
            </div>
        )
